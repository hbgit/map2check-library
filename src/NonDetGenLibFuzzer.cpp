...
///
/// \file
/// This file contains the declaration of the NonDetGenLibFuzzer.cpp class, which 
/// modelling the rules to generate nondet values using LibFuzzer.
///
//===----------------------------------------------------------------------===//
...
short libFuzzerNonDetShort(){
    ...
    if(Map2checkFuzzerSize >= 5){
        ...
        for (size_t cnt = 4; cnt < 6; cnt++) {            
            ...
            // result = 1111111100000000 | 11111111 = 1111111111111111 <- max bits = 65535 
            result = result << 8;
            //
...
int64_t libFuzzerNonDetInt64_t(){
    ...
    if(Map2checkFuzzerSize >= 37){

        for (size_t cnt = 30; cnt < 38; cnt++) {            
            result = result << 8;
            result = result | int64_t(Map2checkFuzzerData[cnt]);
        }
...
float libFuzzerNonDetFloat(){
    ...
    //All four bytes represent a 32 bit float in the reality (byte order little endian).
    // 4 bytes - 0..3

    if(Map2checkFuzzerSize >= 46){
